 ...
 * @version 1.0
 * @Package com.msc.rpc.config
 * @Description: 远程服务引用配置类(客户端)，主要配置以下属性：
 * 1）服务接口名
 * 2）服务接口类类型
 * 3）是否异步调用（默认同步调用）
...
public class ReferenceConfig<T> extends AbstractConfig {
    ...
    /**
     * @return com.msc.rpc.config.ReferenceConfig<T>
     * @Author mSc
     * @Description 创建一个引用配置
     * @Param [interfaceName, interfaceClass, isAsync, isOneWay, isCallback, callbackMethod, callbackParamIndex, timeout, isGeneric, filters]
     **/
     ...
     * @return boolean
     * @Author mSc
     * @Description 判断两个配置是否存在差异
     * @Param [isAsync, isCallback, isOneWay, timeout, callbackMethod, callbackParamIndex, isGeneric, filters]
     **/
     ...
     * @Author mSc
     * @Description 通用调用
     * @Param [methodname, parameterType, args]
     **/
    public Object invokeForGeneric(String methodname, String[] parameterType, Object[] args) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
        if (!initialized) {
            init();
        }
        if (isGeneric) {
     ...
     * @return T
     * @Author mSc
     * @Description 判断是否初始化，未初始化则进行初始化，反之则返回泛型
     * @Param []
     **/
    public T get() {
        if (!initialized) {
    ...
    /**
     * @return com.msc.rpc.config.ReferenceConfig
     * @Author mSc
     * @Description 通过接口名获得引用配置信息
     * @Param [interfaceName]
     **/
